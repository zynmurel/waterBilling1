...
const Home = ({
  ...
  const isGenerate = GetData(
    hostLaravel,
    `api/isGenerate/${prev.getFullYear()}/${month[prev.getMonth()].slice(0, 3)}`
  );
  const {
    data: regen,
    isPending: regenIsPending,
    error: regenError,
    reload: regenReload,
    setReload: setRegenReload,
  } = isGenerate;
  ...
  const handleSubmit = () => {
    ...
    axios
      .put(`${hostLaravel}/api/generateDelinquents`, [], { headers })
      .then((response) => {
    ...
    <Box className="home" sx={{ ...styles.home }}>
      <Box className="container" sx={{ ...styles.container }}>
        ...
        <Box style={{ display: "flex", flexDirection: "row" }}>
            ...
            <Card style={styles.box2}>
              <Box style={styles.box2_1}>
                <Typography
                  gutterBottom
                  fontWeight={"bold"}
                  fontSize={styles.boxFont.fontSize}
                  style={{
                    justifyContent: "center",
                    alignItems: "center",
                    margin: 0,
                ...
                <Typography
                  ...
                  fontWeight={"bold"}
                  fontSize={styles.boxFont.fontSize}
                  sx={{ color: styles.boxFont.color }}
                >
                  {consumer &&
                    !consumerPending &&
                    consumer.consumerReport.totalConnected}
          ...
          <Box>
            <p style={{ margin: 0, width: 380, marginTop: 0 }}>
              Generate Delinquent :
            </p>
            <Button
              variant="contained"
              style={{
                ...
                backgroundColor:
                  regenIsPending ||
                  isGenerate.data === null ||
                  isGenerate.data.delinquents.length === 0
                    ? "#CBCBCB"
                    : "orange",
                fontSize: 18,
                padding: "18px 10px",
            ...
            >
              Generate Delinquent/s for {month[prev.getMonth()].slice(0, 3)}{" "}
              {prev.getFullYear()}
            </Button>
            ...
            <Card style={styles.box2}>
              <Box style={styles.box2_1}>
                ...
                >
                  ...
                  />{" "}
                  {month[prev.getMonth()].slice(0, 3)} {prev.getFullYear()} :
                </Typography>
                <Typography
                  gutterBottom
                  fontWeight={"bold"}
                  ...
                  sx={{ color: styles.boxFont.color }}
                >
                  {collection && !collectionPending
                    ? `₱ ${collection.collectionReport.totalCollection}`
                    : ""}
                  {collectionPending && (
                    <span style={styles.isPending}>...</span>
                  )}
        ...
        >
          <Box sx={{ ...styles.style, width: 550, color: "black" }}>
            <h2 id="parent-modal-title">
              Delinquent Consumer/s of {month[prev.getMonth()].slice(0, 3)}{" "}
              {prev.getFullYear()}
            </h2>
                      ...
                      .map((del, index) => (
                        ...
                        >
                          ...
                          <TableCell style={{ width: 50 }} align="left">
                            {del.consumer_id.toString().padStart(6, "0")}
                          </TableCell>
            ...
            <TablePagination
              ...
              count={
                ...
                isGenerate.data !== null &&
                isGenerate.data.delinquents.length !== 0
                  ? isGenerate.data.delinquents.length
                  : 0
              }
            ...
            >
              ...
              <Button
                style={{ ...styles.generateButton, backgroundColor: "orange" }}
                onClick={() => {
                  setReload(!reload);
                  handleSubmit();
                  setRegenReload(!regenReload);
